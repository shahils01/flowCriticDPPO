"""Single-agent Gymnasium runner for clean PPO/DPPO experiments."""
...
class MujocoRunner(Runner):
    def run(self):
        ...
        for update in range(num_updates):
            ...
            for step in range(self.episode_length):
                values, entropy_lengths, actions, log_probs = self.collect(step)
                ...
                terminated = np.asarray(terminated).reshape(
                    self.n_rollout_threads, -1
                ).all(axis=1, keepdims=True)
                truncated = np.asarray(truncated).reshape(
                    self.n_rollout_threads, -1
                ).all(axis=1, keepdims=True)
    ...
    def warmup(self):
        self.buffer.set_step_obs(0, self.envs.reset())
    ...
    def eval(self, total_num_steps):
        ...
        num_envs = self.all_args.n_eval_rollout_threads
        ...
        while len(episode_rewards) < self.all_args.eval_episodes:
            actions = _to_numpy(
                self.trainer.policy.act(observations, masks, deterministic=True)
            )
            observations, rewards, terminated, truncated, _ = self.eval_envs.step(
                actions
            )
            rewards = np.asarray(rewards).reshape(num_envs, -1).mean(axis=1)
            dones = np.logical_or(terminated, truncated).reshape(num_envs, -1).all(axis=1)
